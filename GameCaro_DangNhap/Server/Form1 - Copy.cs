...
namespace Server
{
    public partial class Form1 : Form
    {
        ...
        {
            serverSocket = socket();
            IPEndPoint ipee = new IPEndPoint(IPAddress.Parse("127.0.0.1"), 8080);
            serverSocket.Bind(ipee);
            ...
            while (true)
            {
                ...
                while (clientSocket.Connected)
                {
                    ...
                    if (signal == "Kientuong")
                    {
                        string temp = "KienTuong";
                        string query = "select * from DANHSACHPHONG where LOAIBAN = '" + temp + "'";
                        connection = new SqlConnection(str);
                        loadData(query);
                        send_Kientuong_Serial();
                        notify = "g";
                        byte[] data = Encoding.ASCII.GetBytes(notify);
                        clientSocket.Send(data);
                        clientSocket.Close();
                    ...
                    if (signal == "TimPhong")
                    {
                        ...
                        string query = "select * from DANHSACHPHONG where IDPHONG = '" + temp + "'";
                        connection = new SqlConnection(str);
                        loadData(query);
                        send_TimPhong_Serial();
                        notify = "h";
                        byte[] data = Encoding.ASCII.GetBytes(notify);
                        clientSocket.Send(data);
                        clientSocket.Close();
